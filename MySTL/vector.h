#ifndef __MYSTL_VECTOR_H
#define __MYSTL_VECTOR_H
...
public:
    ...
    reverse_iterator rbegin() {
        return reverse_iterator(finish_);       // 构造函数
    }
    const_reverse_iterator crbegin() {
        return const_reverse_iterator(finish_); // 构造函数
    }
...
private:
    ...
    template<class InputIterator>
    void vector_aux(InputIterator first, InputIterator last, std::false_type); // std????
    ...
    void reallocate_and_fill(iterator position, const size_type& n, const value_type& value);
    size_type get_new_capacity(size_type n) const;

public:
    template<class Ta, class Alloca>
    friend bool operator==(const vector<Ta, Alloca>& v1, const vector<Ta, Alloca>& v2);
    template<class Ta, class Alloca>
    friend bool operator!=(const vector<Ta, Alloca>& v1, const vector<Ta, Alloca>& v2);
